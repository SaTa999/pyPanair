...
def main(x1, x2, y1, y2, y3, aoas=(7.42), target_dir=""):
    ...
    wgs.create_wgs(os.path.join(target_dir, "ADODG3.wgs"))

    span = halfspan_wing * 2
    sref = chord_wing * span
    xref = chord_wing * 0.25
    aux_name = os.path.join(target_dir, "ADODG3.aux")
    wgs.create_aux(aux_name, alpha=aoas, mach=0.5, cbar=chord_wing, span=span, sref=sref,
                   xref=xref, zref=0.)
